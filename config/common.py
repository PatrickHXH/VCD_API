...
def cookies_headers_gzh(account:str,password:str):
    ...
    headers = {
            "Referer": "http://test.chebufan.cn/chebftest/wx/xlc/view/login_pwd?qd=VCHEDIAN&phone=",
            "Content-Type":"application/x-www-form-urlencoded; charset=UTF-8",
            "qd":"VCHEDIAN",
    }
    login = requests.post(url_login, data=data,headers=headers)
    ...
    headers_gzh = {"Content-Type": "application/json;charset=UTF-8",
                            ...
                            # "User-Agent": "Mozilla / 5.0(Linux;Android6.0;Nexus5Build / MRA58N) AppleWebKit / 537.36(KHTML, likeGecko) Chrome / 102.0.5005.63MobileSafari / 537.36Edg / 102.0.1245.33",
                            # "Cookie":"SESSION=%s"%(login.cookies['SESSION'])
                            }
...
def vpt_header():
    ...
    '''
    null = None
    url = "https://vpttest.chebufan.cn/admin-vpt/api/user/login"
    json = {"timestamp":1655258830751,"sign":"nosign","data":{"loginType":0,"agentId":"admin","agentAuth":"8943e202da22b47012a91c74a0c4fccd","imgValidCodeId":null,"imgValidCodeText":""}}
    headers = {"Content-Type": 'application/json;charset=UTF-8'}
    resp = requests.post(url,json=json,headers=headers)
    vpt_token = resp.json()["data"]["token"]
    vpt_header = {"Content-Type": 'application/json;charset=UTF-8',"X-Token":vpt_token}
    return vpt_header